...
def _is_annotation_type(t: Any) -> bool:
    return type(t) is type and issubclass(t, Annotation)
...
def _is_tuple_of_annotation_types(t: Any) -> bool:
    ...
    if typing.get_origin(t) == tuple and _is_annotation_type(type_args[0]):
        if not (
            ...
            raise TypeError(
                "only tuples that do not mix Annotations with other types are supported"
            )
        return True
    else:
        return False
...
def annotation_field(
    ...
    if target is not None:
        if targets is not None or named_targets is not None:
            raise ValueError("only one of target, targets or named_targets can be set")
        new_targets = [target]
...
class Annotation:
    ...
    def set_targets(self, value: Optional[Tuple[TARGET_TYPE, ...]]):
        if value is not None and self._targets is not None:
            raise ValueError(
                ...
                "annotation list container or remove the annotation with pop() "
                "to assign it to a new annotation list with other targets."
            )
    ...
    def target(self) -> TARGET_TYPE:
        ...
        if len(targets) == 0:
            raise ValueError("annotation has no target")
        if len(targets) > 1:
            raise ValueError("annotation has multiple targets, target is not defined in this case")
        return targets[0]
    ...
    def targets(self) -> Tuple[TARGET_TYPE, ...]:
        if self._targets is None:
            raise ValueError(
                "targets is not set (this annotation may be not yet attached to a document)"
            )
    ...
    def named_targets(self) -> Dict[str, TARGET_TYPE]:
        ...
        if self.TARGET_NAMES is None:
            raise TypeError("no TARGET_NAMES defined")
...
class Document(Mapping[str, Any]):
    ...
    def target_name(cls, field_name: str) -> str:
        """Return the single target name of an annotation field.

        Raise an error if there is not exactly one target.
        """
    ...
    def add_all_annotations_from_other(
        ...
        """Adds all annotations from another document to this document. It allows to blacklist
        annotations and also to override annotations. It returns the original annotations for which
        a new annotation was added to the current document.
    ...
    def deduplicate_annotations(
        ...
        """Deduplicates annotations in the document.

        The method is useful, for instance, if annotations are added by window-based processing
        with overlaps which may lead to duplicated annotations.
        """
